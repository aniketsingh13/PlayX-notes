     ...
     <section className='notecard-Container' style={{backgroundColor: noteColor}}>
      ...
      <small className="   p-xss">
        ...
        {`${new Date(CreatedAt).toLocaleDateString()} ${new Date(CreatedAt).toLocaleString("en-Us",{
          ...
          <div >
              <button className="notecard-btn f-m pr-s" style={{backgroundColor: noteColor}} onClick={() => noteDispatch({type: "RESTOR_TRASHNOTE",payload: trash})} title="Restore trash"><MdOutlineRestoreFromTrash/></button>
          <button className="notecard-btn f-m pr-s" style={{backgroundColor: noteColor}} title='delete forever'  onClick={() => noteDispatch({type: 'DELETE_FOREVER', payload: _id})}><MdDelete /></button>
         </div>
        </div>
        <div className="flex">
