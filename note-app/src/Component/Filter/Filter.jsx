...
const Filter = () => {
  ...
  return (
    <div>
        <button className='filter_Heading f-m font-l mt-l' onClick={() => setFilterToggle(prev => !prev)}>Sort By</button>
        {filterToggle && (
          <div className='filterContainer flex'>
           ...
           <div className='priority_container flex'>
             <label >
              <input value="High" 
              onChange={(e) => filterDispatch({type: "SORT_BY_PRIORITIES",payload: e.target.value})}
              type="checkbox"
           ...
           <div className='tag_container flex'>
            <label>
              <input 
              value="Work"
              onChange={(e) => filterDispatch({type: "SORT_BY_TAGS",payload: e.target.value})}
              type='checkbox'
           ...
           <button className='filter-btn p-xss' onClick={() => filterDispatch({type: "CLEAR_ALL"})}>Clear All</button>
          </div>
        )}
    </div>
  )
